...
class TipoClasificacionModosService:
    def __init__(self, db: Session, schema: str):
        self.db = db
        self.schema = schema
        self.table = get_TipoClasificacionModos_table(schema)
        self.table2 = get_catalogomodoXtipoclasificacion_table(schema)
        
    def all(self, id_modo):
        return (
            self.db.query(self.table)
            .join(self.table2)
            .filter(self.table2.id_modo == id_modo,
...
# servicio para listar  los registros
    ...
    def show(self, tipo_clacificacion_id: int):
        entity = self.db.query(self.table).filter(
            self.table.id == tipo_clacificacion_id,
    ...
    def update_tipo_clasificacion(self, tipo_clasificacion_id: int, 
        ...
        dataupdate = self.db.query(self.table).filter(
            self.table.id == tipo_clasificacion_id,
                self.table.activo == True).first()
        if payload.nombre:
            existe = (
                self.db.query(self.table)
                .filter(self.table.nombre == payload.nombre, self.table.id != tipo_clasificacion_id)
                .first()
            )
            if existe:
                raise HTTPException(
        ...
        try:
            dataupdate = (
                self.db.query(self.table)
                .filter(self.table.id == tipo_clasificacion_id, self.table.activo == True)
                .first()
            )
    ...
    def delete_tipo_clasificacion(self, tipo_clasificacion_id: int, request: Request, tokenpayload: dict):
        ...
        try:
            datadelete = self.db.query(self.table).filter(self.table.id == tipo_clasificacion_id, self.table.activo == True).first()
            if not datadelete:
    ...
    def reactivate(self, tipo_clasificacion_id: int, request: Request, tokenpayload: dict):
        datareactivate = self.db.query(self.table).filter(
            self.table.id == tipo_clasificacion_id).first()
        ...
        try:
            ...
            datareactivate = self.db.query(self.table).filter(self.table.id == tipo_clasificacion_id).first()
            if not datareactivate:
                return {"detail": "Registro no encontrado"}
        # le paso un valor false para realizar un sofdelete para un eliminado logico

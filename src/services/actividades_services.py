...
class ActividadeService:
    ...
    def all(self):
        return (
            self.db.query(actividadesAika)
            .filter(actividadesAika.activo == True)
            .order_by(asc(actividadesAika.nombre))
            .all()
        )
...
# servicio para listar  los registros
    def listar(self, skip: int, limit: int, filtros: str | None = None,
        ...
        query = self.db.query(actividadesAika)
        if activo is not None:
            query = query.filter(actividadesAika.activo == activo)
        
        if filtros:
            query = query.filter(actividadesAika.nombre.ilike(f"%{filtros}%"))
        
        return ( query.order_by(asc(actividadesAika.nombre))
                .offset(skip)
                .limit(limit)
                .all()
                )
    ...
    def create(self, payload: AlcanceCreate, 
        ...
        datacreate = self.db[0].query(actividadesAika).filter(
            actividadesAika.nombre == payload.nombre).first()
        if datacreate:
        ...
        modelos = [actividadesAika, actividadesWayra]
    ...
    def show(self, actividad_id: int):
        entity = self.db.query(actividadesAika).filter(
            actividadesAika.id == actividad_id,
                actividadesAika.activo == True).first()
        if not entity:
    ...
    def updates(self, actividad_id: int, 
                            ...
                            request: Request, tokenpayload: dict):
        dataupdate = self.db[0].query(actividadesAika).filter(
            actividadesAika.id == actividad_id,
                actividadesAika.activo == True).first()
        if payload.nombre:
            existe = (
                self.db[0].query(actividadesAika)
                .filter(actividadesAika.nombre == payload.nombre, actividadesAika.id != actividad_id)
                .first()
            )
            if existe:
                raise HTTPException(
        ...
        for modelo, db in zip(modelos, self.db):
            try:
                dataupdate = (
                    db.query(modelo)
                    .filter(modelo.id == actividad_id, modelo.activo == True)
                    .first()
                )
    ...
    def deletes(self, actividad_id: int, request: Request, tokenpayload: dict):
        datadelete = self.db[0].query(actividadesAika).filter(
            actividadesAika.id == actividad_id,
                actividadesAika.activo == True).first()
        if not datadelete:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="El registro no fue hallada")
        ...
        for modelo, db in zip(modelos, self.db):
            try:
                datadelete = db.query(modelo).filter(modelo.id == actividad_id, modelo.activo == True).first()
                if not datadelete:
                    continue
    ...
    def reactivate(self, actividad_id: int, request: Request, tokenpayload: dict):
        datareactivate = self.db[0].query(actividadesAika).filter(
            actividadesAika.id == actividad_id).first()
        ...
        for modelo, db in zip(modelos, self.db):
            try:
                ...
                datareactivate = db.query(modelo).filter(modelo.id == actividad_id).first()
                if not datareactivate:
                    continue
            # le paso un valor false para realizar un sofdelete para un eliminado logico

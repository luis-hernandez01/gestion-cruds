...
from src.config.dinamic_tables import get_Categoria_fuentefija_table
...
class Categoria_fuentesfijas_Service:
    def __init__(self, db: Session, schema: str):
        self.db = db
        self.schema = schema
        self.table = get_Categoria_fuentefija_table(schema)
...
# servicio para listar  los registros
    ...
    def updates(self, presenta_id: int, 
        ...
        if payload.nombre:
            existe = (
                self.db.query(self.table)
                .filter(self.table.nombre == payload.nombre, self.table.id != presenta_id)
                .first()
            )
            if existe:
                raise HTTPException(
        ...
        try:
            dataupdate = (
                self.db.query(self.table)
                .filter(self.table.id == presenta_id, self.table.activo == True)
                .first()
            )
    ...
    def deletes(self, presenta_id: int, request: Request, tokenpayload: dict):
        ...
        try:
            datadelete = self.db.query(self.table).filter(self.table.id == presenta_id, self.table.activo == True).first()
            if not datadelete:
    ...
    def reactivate(self, presenta_id: int, request: Request, tokenpayload: dict):
        datareactivate = self.db.query(self.table).filter(
            self.table.id == presenta_id).first()
        ...
        try:
            ...
            datareactivate = self.db.query(self.table).filter(self.table.id == presenta_id).first()
            if not datareactivate:
                return {"detail": "Registro no encontrado"}
        # le paso un valor false para realizar un sofdelete para un eliminado logico

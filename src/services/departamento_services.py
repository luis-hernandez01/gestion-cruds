...
# servicio para listar  los registros
    ...
    def create_departamento(self, payload: DepartamentoCreate, 
        ...
        try:
            entity = self.table(nombre=payload.nombre,  codigo = payload.codigo,
    ...
    def update_departamento(self, departamento_id: int, 
        ...
        if payload.nombre:
            existe = (
                self.db.query(self.table)
                .filter(self.table.nombre == payload.nombre, self.table.id != departamento_id)
                .first()
            )
            if existe:
                raise HTTPException(
        ...
        try:
            dataupdate = (
                self.db.query(self.table)
                .filter(self.table.id == departamento_id, self.table.activo == True)
                .first()
            )
    ...
    def delete_departamento(self, departamento_id: int, request: Request, tokenpayload: dict):
        ...
        try:
            datadelete = self.db.query(self.table).filter(self.table.id == departamento_id, self.table.activo == True).first()
            if not datadelete:
    ...
    def reactivate(self, departamento_id: int, request: Request, tokenpayload: dict):
        datareactivate = self.db.query(self.table).filter(
            self.table.id == departamento_id).first()
        ...
        try:
            ...
            datareactivate = self.db.query(self.table).filter(self.table.id == departamento_id).first()
            if not datareactivate:
                return {"detail": "Registro no encontrado"}
# le paso un valor false para realizar un sofdelete para un eliminado logico

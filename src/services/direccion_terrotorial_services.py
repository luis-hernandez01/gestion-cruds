...
from src.config.dinamic_tables import get_direcciones_territoriales_table
...
class DireccionterritorialService:
    def __init__(self, db: Session, schema: str):
        self.db = db
        self.schema = schema
        self.table = get_direcciones_territoriales_table(schema)
...
# servicio para listar  los registros
    ...
    def update_direccion(self, direccion_id: int, 
        ...
        if payload.nombre:
            existe = (
                self.db.query(self.table)
                .filter(self.table.nombre == payload.nombre, self.table.id != direccion_id)
                .first()
            )
            if existe:
                raise HTTPException(
        ...
        try:
            dataupdate = (
                self.db.query(self.table)
                .filter(self.table.id == direccion_id, self.table.activo == True)
                .first()
            )
    ...
    def delete_direccion(self, direccion_id: int, request: Request, tokenpayload: dict):
        ...
        try:
            datadelete = self.db.query(self.table).filter(self.table.id == direccion_id, self.table.activo == True).first()
            if not datadelete:
    ...
    def reactivate(self, direccion_id: int, request: Request, tokenpayload: dict):
        datareactivate = self.db.query(self.table).filter(
            self.table.id == direccion_id).first()
        ...
        try:
            datareactivate = self.db.query(self.table).filter(self.table.id == direccion_id).first()
            if not datareactivate:
                return {"detail": "Registro no encontrado"}
        # le paso un valor false para realizar un sofdelete para un eliminado logico

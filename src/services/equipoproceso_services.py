...
from src.schemas.Equipoproceso_schema import equipoprocesoCreate, equipoprocesoUpdate, LogEntityRead
from datetime import datetime
from src.utils.logs_util import LogUtil
from sqlalchemy import asc
from src.config.dinamic_tables import get_Equipoproceso_table
from src.config.dinamic_tables import get_Tipofuente_table

# Servicio para listar las unidades de ejecucion
class equipoprocesoService:
    def __init__(self, db: Session, schema: str):
        self.db = db
        self.schema = schema
        self.table = get_Equipoproceso_table(schema)
        self.table2 = get_Tipofuente_table(schema)
        
    def all(self, id_tipo_fuente):
        return (
            self.db.query(self.table)
            .join(self.table2)
            .filter(self.table2.id == id_tipo_fuente,
...
# servicio para listar  los registros
    ...
    def create(self, payload: equipoprocesoCreate, 
        ...
        try:
            entity = self.table(nombre=payload.nombre, id_persona=tokenpayload.get("sub"), 
                            id_tipo_fuente=payload.id_tipo_fuente, activo=True, created_at=datetime.utcnow())
            self.db.add(entity)
            self.db.commit()
    ...
    def updates(self, actividad_id: int, 
        ...
        try:
            ...
            if dataupdate:
                ...
                dataupdate.id_tipo_fuente=payload.id_tipo_fuente,
                self.db.commit()

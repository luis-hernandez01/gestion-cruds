...
from src.schemas.factores_emision_schema import FactoremisionCreate, FactoremisionUpdate, LogEntityRead
from datetime import datetime
from src.utils.logs_util import LogUtil
from sqlalchemy import asc
from src.config.dinamic_tables import get_factores_emision_table

# Servicio para listar las unidades de ejecucion
class FactoremisionService:
    def __init__(self, db: Session, schema: str):
        self.db = db
        self.schema = schema
        self.table = get_factores_emision_table(schema)
        
    def all(self):
        return (
            self.db.query(self.table)
            .filter(self.table.activo == True)
            .order_by(asc(self.table.id))
            .all()
        )
...
# servicio para listar  los registros
    ...
    def create(self, payload: FactoremisionCreate, 
                            request: Request, tokenpayload: dict):
        def nullify(value):
            return value if value not in ("", None, "null", "NULL") else None

        
        try:
            entity = self.table(id_tipofuente = nullify(payload.id_tipofuente),
                            id_equipoproceso = nullify(payload.id_equipoproceso),  # ← corregido
                            id_categoria = nullify(payload.id_categoria),
                            id_Combustible = nullify(payload.id_Combustible),

                            N2O_kg_gal = nullify(payload.N2O_kg_gal),
                            CO2_kg_gal = nullify(payload.CO2_kg_gal),
                            CH4_kg_km = nullify(payload.CH4_kg_km),

                            id_unidades_factor_emision = nullify(payload.id_unidades_factor_emision),
        ...
        LogUtil(self.db, self.schema).registrar_log(
            tabla_afectada="factores_emision",
    ...
    def updates(self, actividad_id: int, 
        ...
        try:
            ...
            if dataupdate:
                dataupdate.id_tipofuente = payload.id_tipofuente
                dataupdate.id_equipoproceso = payload.id_equipoproceso
                dataupdate.id_categoria = payload.id_categoria
                dataupdate.id_Combustible = payload.id_Combustible
                dataupdate.id_unidades_factor_emision = payload.id_unidades_factor_emision
                dataupdate.N2O_kg_gal = payload.N2O_kg_gal
                dataupdate.CO2_kg_gal = payload.CO2_kg_gal
                dataupdate.CH4_kg_km = payload.CH4_kg_km

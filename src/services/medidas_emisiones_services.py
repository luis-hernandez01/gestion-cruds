...
from src.schemas.medidas_emisiones_schema import medidasemisionCreate, medidasemisionUpdate, LogEntityRead
from datetime import datetime
from src.utils.logs_util import LogUtil
from sqlalchemy import asc
from src.config.dinamic_tables import get_medidas_emisiones_table

# Servicio para listar las unidades de ejecucion
class MedidasemisionesService:
    def __init__(self, db: Session, schema: str):
        self.db = db
        self.schema = schema
        self.table = get_medidas_emisiones_table(schema)
        
    def all(self):
        return (
            self.db.query(self.table)
            .filter(self.table.activo == True)
            .order_by(asc(self.table.valores))
            .all()
        )
...
# servicio para listar  los registros
    def listar(self, skip: int, limit: int, filtros: str | None = None,
        ...
        if filtros:
            query = query.filter(self.table.valores.ilike(f"%{filtros}%"))
        
        return ( query.order_by(asc(self.table.valores))
                .offset(skip)
                .limit(limit)
                .all()
                )
    ...
    def create(self, payload: medidasemisionCreate, 
                            request: Request, tokenpayload: dict):
        datacreate = self.db.query(self.table).filter(
            self.table.valores == payload.valores).first()
        if datacreate:
        ...
        if payload.valores =="":
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="El campo valores se encuentra vacia ingresa un dato valido")
        # if len(payload.valores) > 255:
        #     raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="El campo valores no puede tener un rango mayor a 255 caracteres")
        
        try:
            entity = self.table(valores=payload.valores, id_tipomedida=payload.id_tipomedida, id_persona=tokenpayload.get("sub"), 
        ...
        LogUtil(self.db, self.schema).registrar_log(
            tabla_afectada="medidas_emisiones",
    ...
    def updates(self, actividad_id: int, 
        ...
        if payload.valores:
            existe = (
                self.db.query(self.table)
                .filter(self.table.valores == payload.valores, self.table.id != actividad_id)
                .first()
            )
            if existe:
                raise HTTPException(
                    status_code=status.HTTP_409_CONFLICT,
                    detail=f"El valores '{payload.valores}' ya está siendo usado por otro registro."
                )
        ...
        try:
            ...
            if dataupdate:
                dataupdate.valores = payload.valores
                dataupdate.id_tipomedida = payload.id_tipomedida

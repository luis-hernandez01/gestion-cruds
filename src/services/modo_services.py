...
class ModoService:
    def __init__(self, db: Session, schema: str):
        self.db = db
        self.schema = schema
        self.table = get_modo_table(schema)
...
# servicio para listar  los registros
    ...
    def update_modo(self, modo_id: int, 
        ...
        if payload.nombre:
            existe = (
                self.db.query(self.table)
                .filter(self.table.nombre == payload.nombre, self.table.id != modo_id)
                .first()
            )
            if existe:
                raise HTTPException(
        ...
        try:
            dataupdate = self.db.query(self.table).filter(self.table.id == modo_id, self.table.activo == True).first()
            if not dataupdate:
    ...
    def delete_modo(self, modo_id: int, request: Request, tokenpayload: dict):
        ...
        try:
            datadelete = self.db.query(self.table).filter(self.table.id == modo_id, self.table.activo == True).first()
            if not datadelete:
                return {"detail": "Registro no encontrado"}
    ...
    def reactivate(self, modo_id: int, request: Request, tokenpayload: dict):
        ...
        try:
            datareactivate = self.db.query(self.table).filter(self.table.id == modo_id).first()
            if not datareactivate:
                return {"detail": "Registro no encontrado"}
        # le paso un valor false para realizar un sofdelete para un eliminado logico

...
from src.config.dinamic_tables import get_unidad_ejecutora_table
...
class UnidadEjecutoraService:
    def __init__(self, db: Session, schema: str):
        self.db = db
        self.schema = schema
        self.table = get_unidad_ejecutora_table(schema)
...
#     # servicio para crear un registro
    ...
    def show(self, unidad_id: int):
        entity = (
            self.db.query(self.table)
            .filter(self.table.id == unidad_id, self.table.activo == True)
            .first()
        )
        if not entity:
            raise HTTPException(
...
#     # servicio para editar logicamente un registro
    def update_unidad(
        ...
        try:
            dataupdate = (
                self.db.query(self.table)
                .filter(self.table.id == unidad_id, self.table.tablelo.activo == True)
                .first()
            )
    ...
    def delete_unidad(self, unidad_id: int, request: Request, tokenpayload: dict):
        ...
        try:
            registro = self.db.query(self.table).filter(self.table.id == unidad_id, self.table.activo == True).first()
            if not registro:
...
# servicio para reactivar logicamente un registro
    def reactivate(self, unidad_id: int, request: Request, tokenpayload: dict):
        ...
        try:
            registro = self.db.query(self.table).filter(self.table.id == unidad_id).first()
            if not registro:
                return {"detail": "Registro no encontrado"}
        # le paso un valor false para realizar un sofdelete para un eliminado logico
